...
namespace ModelGraph.Core
{
    public partial class Root
    {
        ...
        #region GetIdentity  ==================================================
        public string GetIdentity(Item item, IdentityStyle style) => InvalidItem;
        #endregion
        ...
        //        if (qx.IsRoot)
        //        {
        //            if (StoreOf_QueryX.TryGetParent(qx, out Store st))
        ...
        //        var name2 = StoreOf_ColumnX.TryGetParent(cx, out Store tx) ? tx.Name : null;
        //        var noName2 = string.IsNullOrWhiteSpace(name2);
        ...
        //        var name2 = (StoreOf_ComputeX.TryGetParent(cx, out Store pa)) ? GetIdentity(pa, IdentityStyle.Single) : null;
        //        var noName2 = string.IsNullOrWhiteSpace(name2);
        //        if (noName2) name2 = $"{_localize(GetKindKey(IdKey.TableX))} {BlankName}";
        ...
        //        if (!StoreOf_Property.TryGetParent(itm, out Store pa))
        ...
        {
            if (Get<Relation_Store_NameProperty>().TryGetChild(row.Owner, out Property prop))
            {
        ...
        {
            if (Get<Relation_Store_SummaryProperty>().TryGetChild(row.Owner, out Property prop))
            {
                return prop.Value.GetString(row);
            }
            return null;
